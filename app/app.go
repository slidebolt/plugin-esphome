...
func (a *App) resolveAPIKey(dev *internalmdns.Device) string {
	...
	if raw, err := a.store.ReadFile(storage.Internal, devKey); err == nil && len(raw) > 0 {
...
func (a *App) connectAndRegister(ctx context.Context, dev *internalmdns.Device) {
	...
	entityKeyMap := make(map[uint32]entityInfo)
			...
			default:
				...
				if ok {
					if err := a.store.Save(entity); err != nil {
						...
						entityKeyMap[espKey] = entityInfo{id: entity.ID, typ: entity.Type}
						created++
					}
				}
			}
		} else {
			a.applyStateUpdate(dev.Name, entityKeyMap, msg)
		}
	}
	...
	for espKey, info := range entityKeyMap {
		storageKey := PluginID + "." + dev.Name + "." + info.id
		a.conns.Store(storageKey, &entityConn{
			send:   espClient.Send,
			espKey: uint32(espKey),
		})
	}
	defer func() {
		for _, info := range entityKeyMap {
			a.conns.Delete(PluginID + "." + dev.Name + "." + info.id)
		}
	}()
...
func (a *App) applyStateUpdate(devName string, entityKeyMap map[uint32]entityInfo, msg proto.Message) {
	var espKey uint32
	var newState any
	...
	case *api.LightStateResponse:
		espKey = resp.Key
		brightness := int(resp.Brightness * 254)
		if brightness > 254 {
			brightness = 254
		}
		newState = domain.Light{Power: resp.State, Brightness: brightness}
	...
	case *api.ClimateStateResponse:
		...
		newState = domain.Climate{
			HVACMode:    mode,
			Temperature: int(resp.TargetTemperature),
		}
	default:
		return
	}
...
func (a *App) updateFromStorage(devName string, info entityInfo, mutate func(*domain.Entity)) {
	eKey := domain.EntityKey{Plugin: PluginID, DeviceID: devName, ID: info.id}
	raw, err := a.store.Get(eKey)
	if err != nil {
		return
...
func (a *App) handleCommand(addr messenger.Address, cmd any) {
	v, ok := a.conns.Load(addr.Key())
	if !ok {
		log.Printf("plugin-esphome: no connection for %s (device may be offline)", addr.Key())
		return
	}
	ec := v.(*entityConn)
	...
	case domain.LightTurnOff:
		...
		if c.Transition != nil {
			...
			r.TransitionLength = uint32(*c.Transition * 1000)
		}
		req = r
	...
	if err := ec.send(req); err != nil {
		log.Printf("plugin-esphome: send command %T to %s: %v", cmd, addr.Key(), err)
	}
}
