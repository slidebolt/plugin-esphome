...
func TestLuaFade_Physical_Integration(t *testing.T) {
	// Load ESPHOME_API_KEY from .env.local so the plugin can authenticate.
	// The key lives in the workspace root .env.local (gitignored).
	if os.Getenv("ESPHOME_API_KEY") == "" {
		if data, err := os.ReadFile("../../.env.local"); err == nil {
			for _, line := range strings.Split(string(data), "\n") {
				line = strings.TrimSpace(line)
				if strings.HasPrefix(line, "ESPHOME_API_KEY=") {
					os.Setenv("ESPHOME_API_KEY", strings.TrimPrefix(line, "ESPHOME_API_KEY="))
					break
				}
			}
		}
...
drained:
	...
	// over the course of the fade — proving commands reached the physical devices.
	// We don't assert exact values since physical lights may clamp or interpolate.
	for _, e := range lights {
		got := readings[e.Key]
		...
		if max <= min {
			if max == 254 {
				// Non-dimmable binary light (e.g. garage door opener) — always at full brightness.
				t.Logf("~ %s: non-dimmable light (always 254), skipping fade assertion", e.Key)
				continue
			}
			t.Errorf("%s: brightness did not change during fade (readings: %v)", e.Key, got[:wantSteps])
			continue
		}
		t.Logf("✓ %s: brightness ranged %d→%d over %v", e.Key, min, max, got[:wantSteps])
	}
}
