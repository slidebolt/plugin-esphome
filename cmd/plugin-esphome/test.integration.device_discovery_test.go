...
//  2. Seeds a per-device apiKey into Internal storage for every device that
...
func TestDeviceDiscovery_Physical_Integration(t *testing.T) {
	...
	// Step 2: seed per-device keys into Internal storage.
	...
	for _, dev := range devices {
		...
		if err := store.WriteFile(storage.Internal, devKey, json.RawMessage(data)); err != nil {
			t.Fatalf("WriteFile for %s: %v", dev.Name, err)
		}
